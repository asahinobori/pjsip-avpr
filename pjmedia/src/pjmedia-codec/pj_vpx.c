...
static pj_status_t pj_vpx_enum_codecs(pjmedia_vid_codec_factory *factory,
    ...
    *count = 0;
    codecs[*count].fmt_id = (pjmedia_format_id)PJMEDIA_FORMAT_PACK('L', 'V', 'P', '8');
    codecs[*count].pt = PJMEDIA_RTP_PT_VP8_WEBM;
    codecs[*count].encoding_name = pj_str("GVP8");
    codecs[*count].encoding_desc = pj_str("WebM Project VP8");
...
static pj_status_t pj_vpx_alloc_codec(pjmedia_vid_codec_factory *factory,
    ...
    if (info->pt != PJMEDIA_RTP_PT_VP8_WEBM) {
        return PJMEDIA_CODEC_EUNSUP;
    }
...
static pj_status_t pj_vpx_codec_decode_whole(pjmedia_vid_codec *codec,
    ...
    uint32_t pos = 0;
    uint32_t plane, y;
    uint8_t* buffer = (uint8_t*)output->buf;
...
static pj_status_t pj_vpx_codec_decode(pjmedia_vid_codec *codec,
    ...
    s = (pj_uint8_t*)packets[0].buf;
    p = (pj_uint8_t*)vpx->dec_buf;
    vpx->dec_frame_len = 0;
    ...
    res = vpx_codec_decode(&vpx->decoder,
            (uint8_t*)vpx->dec_buf, vpx->dec_frame_len,
            0, VPX_DL_REALTIME);
