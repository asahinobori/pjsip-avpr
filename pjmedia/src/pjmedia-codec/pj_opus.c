...
void apply_opus_codec_params(pj_pool_t* pool, pjmedia_codec_param *attr) {
	...
	if (attr->info.clock_rate < 48000) {
        ...
        sprintf(clock_rate_char, "%d", attr->info.clock_rate);
		attr->setting.dec_fmtp.param[attr->setting.dec_fmtp.cnt].name = pj_str(
...
static pj_status_t opus_codec_open(pjmedia_codec *codec,
	...
	opus->psEnc = (OpusEncoder*)pj_pool_zalloc(opus->pool, structSizeBytes);
	...
	opus->psDec = (OpusDecoder*)pj_pool_zalloc(opus->pool, structSizeBytes);
...
static pj_status_t opus_codec_parse(pjmedia_codec *codec, void *pkt,
    ...
    *frame_cnt = opus_packet_parse((const unsigned char*)pkt, pkt_size,
    		&toc,
    		raw_frames,
    		size,
    		&payload_offset);

    samples_per_frame = opus_packet_get_samples_per_frame((const unsigned char*)pkt, opus->externalFs);
...
static pj_status_t opus_codec_decode(pjmedia_codec *codec,
	...
	opus = (struct opus_private*) codec->codec_data;
    frm_cnt = opus_packet_get_nb_frames((const unsigned char*)(input->buf), input->size);
    ...
    if (opus->pkt_info != pkt_info || input->bit_info == 0) {
		...
		opus->dec_buf_sample_per_frame = opus_packet_get_samples_per_frame((const unsigned char*)(input->buf), opus->externalFs);
		...
		opus->dec_buf_size = opus_decode(opus->psDec,
							...
							(opus_int16*)(opus->dec_buf), opus->dec_buf_max_size,
							0 /* decode FEC */);
...
static pj_status_t opus_codec_recover(pjmedia_codec *codec,
	...
	ret = opus_decode(opus->psDec,
			...
			(opus_int16*)(output->buf), frame_size,
			0);
	if (ret < 0) {
